{
  ...
  outputs =
    ...
    let
      ...
      R_by_date =
        ...
        let
          ...
          additional_package_information_plus =
            let
            ...
            builtins.mapAttrs (
              ...
              // {
                repo = "override";
                r = builtins.filter (r_dep: !builtins.elem r_dep r_builtin_packages) (pkg.r or [ ]);
              }
            ) additional_packages;
          additional_packages_name_to_version =
            let
          ...
          pkgs =
            builtins.trace ("nixpkgs version: " + entry.nixpkgs.lib.version)
              (if nix_pkgs_pkgs != null then nix_pkgs_pkgs else entry.nixpkgs).extend
              (
                final: prev: {
          ...
          bc_version = builtins.trace (
            "bioconductor version: " + entry.bioconductor_version
          ) entry.bioconductor_version;
          R = entry.R pkgs;
          flock =
            if builtins.hasAttr "flock" pkgs then
              pkgs.flock
            else
              with pkgs;
          ...
          get_r_dependencies =
            ...
            in
            if (builtins.hasAttr v.pname r_dependency_overrides) then
              let
                  ...
                  buildInputs =
                    map
                      (
                        ...
                        in
                        package_derivations_all_versions.${tagged_dep}
                      ) # lazy recursion for the win
          ...
          package_derivations_this_date = lib.mapAttrs (
            pkg_name: version:
            let
              key = pkg_name + "_" + version;
            in
            (
              if (builtins.hasAttr pkg_name packageOverrideAttrs) then
                (package_derivations_all_versions.${key}.overrideAttrs (packageOverrideAttrs.${pkg_name}))
              else
          ...
          requested_r_packages_filtered =
            if (builtins.isString r_pkg_names && r_pkg_names == "cran") then
              builtins.attrValues (
                lib.filterAttrs (k: v: (!v.broken or false) && v.repo == "cran") package_derivations_this_date
              )
            else if (builtins.isString r_pkg_names && r_pkg_names == "bioc_software") then
              builtins.attrValues (
                lib.filterAttrs (
                  k: v: (!v.broken or false) && v.repo == "bioc_software"
                ) package_derivations_this_date
              )
            else
              map (x: package_derivations_this_date.${x}) r_pkg_names;
          r_wrapper =
            with pkgs;
    ...
    // (lib.mapAttrs' (
      k: v:
      (lib.attrsets.nameValuePair (k + "_nixpkgs") (
    ...
    // (lib.mapAttrs' (
      ...
      lib.attrsets.nameValuePair (k + "_cran") (R_by_date {
        date = k;
        r_pkg_names = "cran";
      })
    ) r_by_date_data)
    // (lib.mapAttrs' (
      k: v:
      # _cran builds all of cran on that date
      lib.attrsets.nameValuePair (k + "_bioc_software") (R_by_date {
        date = k;
