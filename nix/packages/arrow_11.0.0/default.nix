{
  ...
  enableGcs ? (!stdenv.isDarwin) && (lib.versionAtLeast (grpc.cxxStandard or "14") "17"), # google-cloud-cpp is not supported on darwin, needs to support C++17
}:
assert lib.asserts.assertMsg
((enableS3 && stdenv.isDarwin) -> (lib.versionOlder boost.version "1.69" || lib.versionAtLeast boost.version "1.70"))
"S3 on Darwin requires Boost != 1.69"; let
  thrift_plus_libevent = thrift.overrideAttrs (finalAttrs: previousAttrs: {
    propagatedBuildInputs = (previousAttrs.propagatedBuildInputs or []) ++ [libevent];
    patches =
...
in
  stdenv.mkDerivation rec {
    ...
    installCheckInputs =
      ...
      ++ lib.optionals enableFlight [python3]; # older nixpkgs ain't got nativeInstallCheckInputs
